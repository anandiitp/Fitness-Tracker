...
#converted all the above work into a function
files=glob("../../data/raw/MetaMotion/*.csv")
...
# Merging datasets
# --------------------------------------------------------------
merge_data=pd.concat([acc_df.iloc[:,:3],gyr_df],axis=1)
# merge_data.dropna()
#Renameing the coloumns
merge_data.columns=[
    "acc_x",
    "acc_y",
    "acc_z",
...
merge_data.columns
merge_data[:1000].resample(rule="200ms").apply(sampling)

#split by day

days=[g for n, g in merge_data.groupby(pd.Grouper(freq='D'))]
